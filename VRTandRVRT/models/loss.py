...
"""
class UncertainWeighLosses(nn.Module):
    def __init__(self,eps=1e-9):
        super(UncertainWeighLosses, self).__init__()
        self.log_sigma1 = nn.Parameter(torch.tensor(0.0))  # Trainable log variance
    ...
    def forward(self, x, y):
        diff = x - y
        loss_all = torch.mean(torch.sqrt((diff * diff) + self.eps))
        ...
        loss = (1 / (2 * sigma1**2)) * loss_all + (1 / (2 * sigma2**2)) * loss_hf + torch.log(sigma1 * sigma2)
        return loss
...
class LPIPSLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x = x.reshape(-1, c, h, w)
        y = y.reshape(-1, c, h, w)
        frame_losses = self.loss_fn(x, y,normalize=True)
        frame_losses = frame_losses.view(b, t)
        video_losses = frame_losses.mean(dim=1)
        total_loss = video_losses.mean()
        return total_loss
