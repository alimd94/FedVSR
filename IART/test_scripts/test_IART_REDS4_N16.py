...
def main():
    ...
    for subfolder, subfolder_gt in zip(subfolder_l, subfolder_gt_l):
        ...
        for idx in range(outputs.shape[0]):
            ...
            output, img_gt, crop_border=crop_border, test_y_channel=test_y_channel)
            crt_lpips = loss_fn_alex(
                                        (torch.from_numpy(output).permute(2, 0, 1).unsqueeze(0).to(device) * 2 - 1),
            ...
            logger.info(f'{subfolder_name}--{img_name} - PSNR: {crt_psnr:.6f} dB. SSIM: {crt_ssim:.6f} LPIPS: {crt_lpips:.6f}')
            name_idx += 1
    ...
    for folder_idx, subfolder_name in enumerate(subfolder_names):
        logger.info(f'Folder {subfolder_name} - Average PSNR: {avg_psnr_l[folder_idx]:.6f} dB. Average SSIM: {avg_ssim_l[folder_idx]:.6f}. Average LPIPS: {avg_lpips_l[folder_idx]:.6f}')
