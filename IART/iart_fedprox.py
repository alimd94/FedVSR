...
def train_with_proximal(net, trainloader,scaler, epochs):
    current_step = 0 

    global_params = [val.detach().clone() for val in net.net_g.parameters()]
    ...
    for epoch in range(epochs):
        for i, train_data in enumerate(trainloader):
            ...
            with autocast():
                ...
                if RND > 0:
                    proximal_term = 0.0
                    for w, w_t in zip(global_params, net.net_g.parameters()):
                        proximal_term += torch.square((w_t - w).norm(2))
                    l_total = l_total + (PROXIMAL_MU / 2) * proximal_term
...
class FlowerClient(NumPyClient):
    ...
    def fit(self, parameters,config):
        ...
        train_with_control_variates(self.net, self.trainloader,self.scaler,1)
